...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_netbox_importer.compose_http_timeout,
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_netbox_importer.project_name}",
        f'--project-directory "{context.nautobot_netbox_importer.compose_dir}"',
    ]
    ...
    if (
        context.nautobot_netbox_importer.ephemeral_ports
        and context.nautobot_netbox_importer.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_netbox_importer.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_netbox_importer.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_netbox_importer.compose_dir, "creds.env"),
        )
...
@task(
    help={
        "version": "Version of Nautobot NetBox Importer to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_netbox_importer'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_netbox_importer")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_netbox_importer.migrations"
